    ...
    try {
      if (url.pathname === '/api/get-signature' && request.method === 'POST') {
        ...
        return new Response(JSON.stringify({
          rp_id: env.RP_ID, signature: sigData.signature,
          nonce: sigData.nonce, created_at: sigData.created_at,
      ...
      if (url.pathname === '/api/verify-proof' && request.method === 'POST') {
        ...
        const verificationResult = await verifyProof(env.RP_ID, proofPayload);
